...
import { onClose as closeVisitfactoryForm } from "../store/visitFactorySlice";
...
export const FactoryFormInput = ({ register, label, id, required, type, minLength, spanFull, errors, maxLength }) => {
  return (
...
const VisitfactoryForm = () => {
  const { isOpen } = useSelector((state) => state.visitFactory);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const res = await axios.post("/api/contact", { data, type: "factory-visit" });

      if (res.status == 200) {
        toast.success("Message Sent Successfully!")
        dispatch(closeVisitfactoryForm())
      }
  ...
  return (
    isOpen && (
      <div
        onClick={() => dispatch(closeVisitfactoryForm())}
      ...
      >
        ...
        >
          ...
          /> */}
          <span className="text-lg md:text-xl lg:text-3xl text-black text-center">Welcome to Haroth.com Factory
...
export default VisitfactoryForm;
